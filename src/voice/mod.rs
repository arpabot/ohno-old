...
impl VoiceClient {
  ...
  pub async fn speech<T: AsRef<str>>(&self, text: T) -> Result<Bytes> {
    ...
    match self
      .client
      .post(self.baseurl("v1"))
      .header("Content-Type", "application/ssml+xml")
      .header("User-Agent", "OHNO/0.1.0")
      .header(
        "Authorization",
        &*format!("Bearer {token}", token = self.authorize().await.unwrap()),
      )
  ...
  fn baseurl(&self, endpoint: &str) -> String {
    format!(
      "https://{region}.tts.speech.microsoft.com/cognitiveservices/{ep}",
      ep = endpoint,
      region = self.region
    )
  }
      ...
      .await
    {
      Ok(res) => match res.text().await {
        Ok(text) => match serde_json::from_str::<Value>(&text) {
          Ok(json) => match json.get("voices") {
            Some(voicevalue) => match voicevalue {
              Value::Array(voices) => {
